...
## Using R in Shiny for Python

In a Shiny for Python application, you can incorporate R scripts using the 
[rpy2](https://rpy2.github.io/) package.

Below shows an example where we have some R scripts that contains functions we
want to use in our Shiny for Python application
...
We can now use the functions `read_in_data_r` and `do_something_r` in our app.
...
**Some considerations**:
1. This is great method to quickly incorporate some R code into a Shiny for
Python app, or a package which is not available in Python.
...
cases. If your app relies on snappy performance, use this method as
a prototype, and subsequently consider whether the effort of porting your R
scripts to a Python-based method is worth the performance improvement.

## Using Python in Shiny for R
